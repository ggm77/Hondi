package com.seohamin.hondi.global.stomp.error;

import com.seohamin.hondi.domain.chat.dto.error.ChatErrorDto;
import com.seohamin.hondi.global.stomp.constants.StompConstants;
import com.seohamin.hondi.global.util.JsonUtil;
...
public class StompErrorSender {
    ...
    /**
     * 세션에 ERROR 프레임을 보내는 메서드 (연결이 끊어짐)
    ...
    public void toSession(
        ...
        // 4) 메세지 전송
        clientOutboundChannel.getObject().send(payload);
    }

    /**
     * 유저에게 에러 발송하는 메서드 (/user/queue/error 구독 필요)
    ...
    public void toUser(
        ...
        simpMessagingTemplate.getObject()
                .convertAndSendToUser(principal.getName(), StompConstants.DEST_ERROR_MESSAGE, chatErrorDto);
    }
}
